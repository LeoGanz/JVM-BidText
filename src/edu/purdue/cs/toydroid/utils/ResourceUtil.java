...
public class ResourceUtil {
	...
	public static void parse(String apk, ClassHierarchy ich) {
		...
		try {
			...
			ExtFile eFile = new ExtFile(file);
			ResourcesDecoder libRes = new ResourcesDecoder(Config.getDefaultConfig(), eFile);
//			libRes.buildOptions = new BuildOptions();
			// already load main package in getResTable(). calling loadMainPkg()
			// causes errors.
			ResTable resTable = libRes.getResTable();
...
//			frwkPackage = libRes.loadFrameworkPkg(resTable, 1, null);
	...
	private static void parseARSC(ResTable resTable) {
		...
		logger.debug("Parsing Main Package.");
			for (ResResSpec spec : mainPackage.listResSpecs()) {
	...
	private static void parseManifest(ZipFile apkAchive, ZipEntry manifest) {
		...
		try {
			...
			attrDecoder.setResTable(mainPackage.getResTable());
			parser.setAttrDecoder(attrDecoder);
			int type = -1;

			while ((type = parser.next()) != 1) { //XmlPullParser.END_DOCUMENT
				if (type == 2) { //XmlPullParser.START_TAG
...
//			attrDecoder.setCurrentPackage(mainPackage);
			...
			while ((type = parser.next()) != 1) { //XmlPullParser.END_DOCUMENT
				switch (type) {
					case 2: //XmlPullParser.START_TAG
	...
	private static int getAttributeId(AXmlResourceParser parser, String attr) {
		for (int i = 0; i < parser.getAttributeCount(); i++) {
			...
			if (attrName.equals(attr)) {
				return parser.getAttributeValueType(i);// return parser.getAttributeResourceValue(i, 0);
			}
		}
		return 0;
	}
	...
	private static String getAttributeText(AXmlResourceParser parser,
		...
		for (int i = 0; i < parser.getAttributeCount(); i++) {
			...
			if (attr.equals(name)) {
				...
				if (vType == 0x01) { //TypedValue.TYPE_REFERENCE
					int ID = parser.getAttributeValueType(i);
				...
				} else if (vType == 0x03) { //TypedValue.TYPE_STRING
